...
class PyborgDiscord(discord.Client):
    ...
    config: MutableMapping[str, Any] = attr.ib(default=None)
    ...
    def __attrs_post_init__(self) -> None:
        ...
        try:
            ...
            self.multiplexing = self.config['pyborg']['multiplex']
            self.multi_server = self.config['pyborg']['multiplex_server']
            self.multi_port = self.config['pyborg']['multiplex_port']
        except KeyError:
    ...
    def our_start(self) -> None:
        ...
        self.scan()
        if 'token' in self.config['discord']:
    ...
    async def on_message(self, message: discord.Message) -> None:
        ...
        line = " ".join(line_list)
        try:
            if self.config["discord"]["plaintext_ping"]:
        ...
        random_reply = False
        if "reply_chance" in self.config['discord'] and self.config['discord']['reply_chance']:
            random_reply = random.randint(1,100) <= int(self.config['discord']['reply_chance'])

        if random_reply or self.user.mentioned_in(message) or self._plaintext_name(message):
            async with message.channel.typing():
                msg = await self.reply(line, message.author.id == self.config["discord"]["owner_id"])
    ...
    async def learn(self, body: str) -> None:
        ...
        if self.config['pyborg']['multiplex']:
            await self.aio_session.post(
                f"http://{self.multi_server}:{self.multi_port}/learn", data={"body": body},
                raise_for_status=True)

    async def reply(self, body: str, owner: bool) -> Union[str, None]:
        """thin wrapper for reply to switch to multiplex mode: now coroutine"""
        if self.config['pyborg']['multiplex']:
            url = f"http://{self.multi_server}:{self.multi_port}/reply"
            async with self.aio_session.post(url, data={"body": body, "owner": owner}, raise_for_status=True) as ret:
                try:
